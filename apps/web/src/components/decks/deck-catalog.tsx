  ...
  return (
    <>
      ...
      {filtered.length ? <div className="deck-grid catalog-grid">{filtered.map((deck) => <DeckCard key={deck.id} deck={deck} />)}</div> : (
        <div className="data-panel empty-state"><div><div className="empty-icon"><Search size={22} /></div><h3>Nenhum deck encontrado</h3><p>Tente outro nome ou remova os filtros.</p><button className="button-secondary" onClick={() => setQuery("")}>Limpar busca</button></div></div>
      )}
    </>
  );
}
